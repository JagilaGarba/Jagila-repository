...
Select count(*) as NotNullcount from #products where product is not NULL -- count = 29,363
Select count(*) as Nullcount from #products where product is  NULL -- count = 120,657
...
--- Max and Min Items sold (confirming the items sold in each order)
...
------- Average items purchased by customers

Select (Avg(ItemSold)) AverageItemsPurchased from 
(Select Ordernumber, count (Product) as ItemSold
from #products
group by Ordernumber
) as X
...
SELECT
PRODUCT_1, PRODUCT_2, PRODUCT_3, COUNT(ON1) TRANS_COUNT
FROM
(
SELECT
a.Ordernumber as ON1,
